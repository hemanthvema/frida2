...
class RobotMain(object):
    ...
    def __init__(self, robot, **kwargs):
        ...
        # Load last completed sample index from file
        self.last_completed_sample = self.load_last_completed_sample()
    ...
    # Save the last completed sample index to a file
    def save_last_completed_sample(self):
        with open('last_completed_sample.txt', 'w') as file:
            file.write(str(self.last_completed_sample))

    # Load the last completed sample index from a file
    def load_last_completed_sample(self):
        try:
            with open('last_completed_sample.txt', 'r') as file:
...
# Robot Main Run
    def run(self, zero_position = None, paths = None, num_samples = None,  joint_motion_1 = None, joint_motion_2= None , linear_motion_1= None, linear_motion_2 = None ):
        ...
        try:
            ...
            positions_sampleholder = positions_sampleholder[self.last_completed_sample:]

            for steps, position in enumerate(positions_sampleholder, start=self.last_completed_sample + 1):
                ...
                code = self._arm.set_position(*[-29.3, -316.3, 338, 78.4, -87.9, 11.5], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
            ...
            # Update the last completed sample index
            self.last_completed_sample = steps
        ...
        # Save the last completed sample index to a file
        self.save_last_completed_sample()
...
if __name__ == '__main__':
    ...
    robot_main.run(zero_position=[87, 0, 154.2, 180, 0, 0], paths = sp.path_generator.paths, num_samples = 1)
