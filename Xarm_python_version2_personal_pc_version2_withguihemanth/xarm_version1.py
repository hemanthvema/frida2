    ...
    def run(self):
        try:
            ...
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(1)
            code = self._arm.set_position(*[-56.9, 177.2, 154.2, 153.8, -86.0, 115.6], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(1)
            code = self._arm.set_position(*[40.2, 177.2, 113.7, 153.8, -86.0, 115.6], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(1)
            code = self._arm.set_position(*[40.2, 237.4, 113.7, 153.8, -86.0, 115.6], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(5)
            ...
            if not self._check_code(code, 'close_lite6_gripper'):
                return
            time.sleep(5)
            code = self._arm.set_position(*[40.2, 237.4, 450.0, 153.8, -86.0, 115.6], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
            if not self._check_code(code, 'set_position'):
                return
            time.sleep(1)
