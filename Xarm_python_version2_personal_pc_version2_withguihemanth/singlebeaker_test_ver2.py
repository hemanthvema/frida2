...
import traceback
import threading
from Xarm_python_version2_personal_pc_version2_withguihemanth.xarm import version
from Xarm_python_version2_personal_pc_version2_withguihemanth.xarm.wrapper import XArmAPI
import Electrochemistry_Backend
        ...
        try:
            ...
            for i in range(int(num_samples)):
                ...
                if not self._check_code(code, 'set_position'):
                    return
                time.sleep(3)
                code = self._arm.set_position(*[262.1, 76.0, 272.3, -90.0, -90.0, -90.0], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
                if not self._check_code(code, 'set_position'):
                    return
                time.sleep(2)
                code = self._arm.set_position(*[262.1, 76.0, 423.8, -90.0, -90.0, -90.0], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
                if not self._check_code(code, 'set_position'):
                    return
                time.sleep(10)
                code = self._arm.set_position(*[262.1, 76.0, 272.3, -90.0, -90.0, -90.0], speed=self._tcp_speed, mvacc=self._tcp_acc, radius=0.0, wait=True)
